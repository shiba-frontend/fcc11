...
const TeamList = () => {
    ...
    const GetData = async (pagenumber, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `teams?search=${text}`
        } else endpoint = `teams?page=${pagenumber}`
    ...
    const StatusHandler = async () =>{
        ...
        try {
           ...
           data.append('status', status == 1 ? 0 : 1);
            ...
            const  response = await ApiConnection.post(`teams/change-status/${rowId}`, data)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setstatusmodal(false)
    ...
    const DeleteHandler = async () =>{
        ...
        try {
            ...
            const  response = await ApiConnection.delete(`teams/${rowId}`)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setdeletemodal(false)
    ...
    const  DownLoadExcel = async () =>{
        ...
        try{
            const  response = await ApiConnection.get('teams/excel')
            if(response?.status == 200){
  ...
  return (
    <div>
        {pageloading && <Loader/>}
    <DashboardHeader title="Team List" />
    <AdminMenu />
      ...
      <div className="dashboard-panel">
            ...
            <div className='form-group w-50 d-flex'>
                ...
                <div className='form-group'>
                    <ul>
                        <li>
                            <NavLink to="/admin/team-list/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add Team</NavLink>
                        </li>
                        <li>
    ...
    <thead>
        ...
        <tbody>
           ...
           FilterResult&&FilterResult.map((list, index)=>{
            return (
                <tr key={index}>
                <td><img src={BaseUrl.baseurl + list?.team_image} width="40" /></td>
                <td>{list?.team_name}</td>
                <td>{list?.team_short_name}</td>
                <td>{list?.incorporation_year}</td>
                <td>{list?.players?.map((list, i)=>{
                return <span className='badge bg-primary text-white mr-2' key={i}>{list?.player_name} </span>
            })}</td>
                <td>
                    <NavLink to={`/admin/team-list/view/${list?.id}`} className="btn btn-sm btn-outline-primary"><i className="fa-solid fa-eye"></i></NavLink>
                    <NavLink to={`/admin/team-list/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>

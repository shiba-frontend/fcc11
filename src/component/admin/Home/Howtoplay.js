...
const Howtoplay = () => {
    ...
    const GetData = async (pagenumber, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `howtoplay?search=${text}`
        } else endpoint = `howtoplay?page=${pagenumber}`
...
const paginateHandle = (number) =>{
  ...
  const StatusHandler = async () =>{
      ...
      try {
         ...
         data.append('status', status == 1 ? 0 : 1);
          ...
          const  response = await ApiConnection.post(`howtoplay/change-status/${rowId}`, data)
          if(response?.status == 200){
              GetData()
              setloading(false)
              setstatusmodal(false)
  ...
  const DeleteHandler = async () =>{
      ...
      try {
          ...
          const  response = await ApiConnection.delete(`howtoplay/${rowId}`)
          if(response?.status == 200){
              GetData()
              setloading(false)
              setdeletemodal(false)
  ...
  const  DownLoadExcel = async () =>{
      ...
      try{
          const  response = await ApiConnection.get('howtoplay/excel')
          if(response?.status == 200){
  ...
  return (
    <div>
    {pageloading && <Loader/>}
<DashboardHeader title="How to play" />
<AdminMenu />
...
<div className="dashboard-panel">
      ...
      <div className='form-group w-50 d-flex'>
          ...
          <div className='form-group'>
              <ul>
                  <li>
                      <NavLink to="/admin/howtoplay/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add</NavLink>
                  </li>
                  <li>
     ...
     FilterResult&&FilterResult.map((list, index)=>{
      return (
          ...
          <td>
              ...
              <NavLink to={`/admin/howtoplay/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>

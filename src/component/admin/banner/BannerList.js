...
const BannerList = () => {
    ...
    const GetData = async (pagenumber, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `home-banner?search=${text}`
        } else endpoint = `home-banner?page=${pagenumber}`
...
const ResetHandle = ()=>{  
  ...
  const StatusHandler = async () =>{
      ...
      try {
         ...
         data.append('status', status == 1 ? 0 : 1);
          ...
          const  response = await ApiConnection.post(`home-banner/change-status/${rowId}`, data)
          if(response?.status == 200){
              GetData()
              setloading(false)
              setstatusmodal(false)
  ...
  const DeleteHandler = async () =>{
      ...
      try {
          ...
          const  response = await ApiConnection.delete(`home-banner/${rowId}`)
          if(response?.status == 200){
              GetData()
              setloading(false)
              setdeletemodal(false)
  ...
  const  DownLoadExcel = async () =>{
      ...
      try{
          const  response = await ApiConnection.get('home-banner/excel')
          if(response?.status == 200){
  ...
  return (
    <div>
    {pageloading && <Loader/>}
<DashboardHeader title="Banner list" />
<AdminMenu />
...
<div className="dashboard-panel">
      ...
      <div className='form-group w-50 d-flex'>
          ...
          <div className='form-group'>
              <ul>
                  <li>
                      <NavLink to="/admin/banner/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add Banner</NavLink>
                  </li>
                  <li>
  ...
  <tbody>
     ...
     FilterResult&&FilterResult.map((list, index)=>{
      return (
          ...
          <td><img src={BaseUrl.baseurl + list?.banner_image} width="100"  /></td>
          <td>{list?.position}</td>
          ...
          <td>
              ...
              <NavLink to={`/admin/banner/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>

...
const TournamentList = () => {
    ...
    const GetData = async (pagenumber, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `tournament?search=${text}`
        } else endpoint = `tournament?page=${pagenumber}`
    ...
    const StatusHandler = async () =>{
        ...
        try {
           ...
           data.append('status', status == 1 ? 0 : 1);
            ...
            const  response = await ApiConnection.post(`tournament/change-status/${rowId}`, data)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setstatusmodal(false)
    ...
    const DeleteHandler = async () =>{
        ...
        try {
            ...
            const  response = await ApiConnection.delete(`tournament/${rowId}`)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setdeletemodal(false)
    ...
    const  DownLoadExcel = async () =>{
        ...
        try{
            const  response = await ApiConnection.get('tournament/excel')
            if(response?.status == 200){
    ...
    const CompleteHandle = async (status, id)=>{
        setloading(true)
        ...
        try {
           ...
           data.append('status', status);
            ...
            const  response = await ApiConnection.post(`tournament/change-status/${id}`, data)
            if(response?.status == 200){
                GetData()
                setloading(false)
                toast.success(response?.data?.message)
  ...
  return (
    <div>
          {pageloading && <Loader/>}
    <DashboardHeader title="Tournament List" />
    <AdminMenu />
      ...
      <div className="dashboard-panel">
            ...
            <div className='form-group w-50 d-flex'>
                ...
                <div className='form-group'>
                    <ul>
                        <li>
                            <NavLink to="/admin/tournament-list/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add Tournament</NavLink>
                        </li>
                        <li>
        ...
        <tbody>
           ...
           FilterResult&&FilterResult.map((list, index)=>{
            return (
                ...
                <td>{list?.teams?.map((item)=> {
                    return <span>{item?.team_name}, </span>
                })}</td>
                <td>{list?.tournament_short_name}</td>
                <td>{list?.tournament_year}</td>
                <td>
                    <NavLink to={`/admin/tournament-list/view/${list?.id}`} className="btn btn-sm btn-outline-primary"><i className="fa-solid fa-eye"></i></NavLink>
                    <NavLink to={`/admin/tournament-list/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>
                    ...
                    <button onClick={()=>CompleteHandle(3, list?.id)} className={list?.is_active == 3 ? 'btn btn-sm btn-outline-success ml-2' : 'btn btn-sm btn-outline-danger ml-2'} disabled={list?.is_active == 3 ? true:false}>{list?.is_active == 3 ? "Completed" : "Mark to completed"}</button>
                </td>
            </tr>
            )
           })

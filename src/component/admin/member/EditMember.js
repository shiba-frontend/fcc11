...
const EditMember = () => {
    ...
    const fetchdata = async ()=>{
        ...
        try{
            const response = await ApiConnection.get(`members/${id}`)
            setloading(false)  
            if(response.status === 200){
                var editdata = response?.data?.data
                setclub(editdata?.club_id)
...
const AddHandler = async () => {
    ...
    if(fname == ''){
        ...
        try{
            ...
            const response = await ApiConnection.post(`members/${id}`, data);
            if(response.status === 200){
                setloading(false)
  ...
  return (
    <div>
    {loading && <Loader/>}
<DashboardHeader title="Edit member user" />
<AdminMenu />

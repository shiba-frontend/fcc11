...
const AssociationAdminList = () => {
    ...
    const [Totallength, setTotallength] = useState(0)
     ...
     const GetData = async (number, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `club-associate?search=${text}`
        } else endpoint = `club-associate?page=${number}`
    ...
    const StatusHandler = async () =>{
        ...
        try {
           ...
           data.append('status', status == 1 ? 0 : 1);
            ...
            const  response = await ApiConnection.post(`club-associate/change-status/${rowId}`, data)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setstatusmodal(false)
    ...
    const DeleteHandler = async () =>{
        ...
        try {
            ...
            const  response = await ApiConnection.delete(`club-associate/${rowId}`)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setdeletemodal(false)
    ...
    const  DownLoadExcel = async () =>{
        ...
        try{
            const  response = await ApiConnection.get('club-associate/excel')
            if(response?.status == 200){
  ...
  return (
    <div>
         {pageloading && <Loader/>}
    <DashboardHeader title="League Admin List" />
    <AdminMenu />
      ...
      <div className="dashboard-panel">
            ...
            <div className='form-group w-50 d-flex'>
                ...
                <div className='form-group'>
                    <ul>
                        <li>
                            <NavLink to="/admin/association-admin/add" className='btn btn-success' ><i className="fa-solid fa-plus"></i> Add league admin</NavLink>
                        </li>
                        <li>
        ...
        <tbody>
           ...
           FilterResult&&FilterResult.map((list, index)=>{
            return (
                ...
                <td>{list?.phone}</td>
                <td>
                    <NavLink to={`/admin/association-admin/view/${list?.id}`} className="btn btn-sm btn-outline-primary"><i className="fa-solid fa-eye"></i></NavLink>
                    <NavLink to={`/admin/association-admin/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>
...
export default AssociationAdminList
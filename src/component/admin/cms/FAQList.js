...
const FAQList = () => {
    ...
    const GetData = async (pagenumber, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `faq?search=${text}`
        } else endpoint = `faq?page=${pagenumber}`
...
const ResetHandle = ()=>{  
  ...
  const StatusHandler = async () =>{
      ...
      try {
         ...
         data.append('status', status == 1 ? 0 : 1);
          ...
          const  response = await ApiConnection.post(`faq/change-status/${rowId}`, data)
          if(response?.status == 200){
              GetData()
              setloading(false)
              setstatusmodal(false)
  ...
  const DeleteHandler = async () =>{
      ...
      try {
          ...
          const  response = await ApiConnection.delete(`faq/${rowId}`)
          if(response?.status == 200){
              GetData()
              setloading(false)
              setdeletemodal(false)
  ...
  const  DownLoadExcel = async () =>{
      ...
      try{
          const  response = await ApiConnection.get('faq/excel')
          if(response?.status == 200){
...
<div className="dashboard-panel">
      ...
      <div className='form-group w-50 d-flex'>
          ...
          <div className='form-group'>
              <ul>
                  <li>
                      <NavLink to="/admin/faq/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add FAQ</NavLink>
                  </li>
                  <li>
  ...
  <tbody>
     ...
     FilterResult&&FilterResult.map((list, index)=>{
      return (
          ...
          <td>{list?.answer}</td>
          <td>
              <NavLink to={`/admin/faq/view/${list?.id}`} className="btn btn-sm btn-outline-primary"><i className="fa-solid fa-eye"></i></NavLink>
              <NavLink to={`/admin/faq/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>

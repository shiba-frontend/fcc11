...
const SubscriptionList = () => {
    ...
    const GetData = async (number, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `subscription?search=${text}`
        } else endpoint = `subscription?page=${number}`
    ...
    const StatusHandler = async () =>{
        ...
        try {
           ...
           data.append('status', status == 1 ? 0 : 1);
            ...
            const  response = await ApiConnection.post(`subscription/change-status/${rowId}`, data)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setstatusmodal(false)
    ...
    const DeleteHandler = async () =>{
        ...
        try {
            ...
            const  response = await ApiConnection.delete(`subscription/${rowId}`)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setdeletemodal(false)
    ...
    return (
        <div>
    {pageloading && <Loader/>}
<DashboardHeader title="Subscription Plan" />
<AdminMenu />
  ...
  <div className="dashboard-panel">
        ...
        <div className='form-group w-50 d-flex'>
            ...
            <div className='form-group'>
                <ul>
                    <li>
                        <NavLink to="/admin/subscription/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add Subscription</NavLink>
                    </li>
                    <li>
    ...
    <tbody>
       ...
       FilterResult&&FilterResult.map((list, index)=>{
        return (
            ...
            <td>{list?.plan_type?.plan_type}</td>
            <td>{list?.plan_duration}</td>
            <td>{list?.plan_price}</td>
            <td>{list?.credit?.credit}</td>
         
            <td>
                <NavLink to={`/admin/subscription/view/${list?.id}`} className="btn btn-sm btn-outline-primary"><i className="fa-solid fa-eye"></i></NavLink>
                <NavLink to={`/admin/subscription/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>

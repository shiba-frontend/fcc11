...
const FantacyGameAdmin = () => {
    ...
    const GetData = async (number, text)=>{
        ...
        let endpoint = ''
        if(text !== ''){
            endpoint = `fantasy-game-admin?search=${text}`
        } else endpoint = `fantasy-game-admin?page=${number}`
    ...
    const StatusHandler = async () =>{
        ...
        try {
           ...
           data.append('status', status == 1 ? 0 : 1);
            ...
            const  response = await ApiConnection.post(`fantasy-game-admin/change-status/${rowId}`, data)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setstatusmodal(false)
    ...
    const DeleteHandler = async () =>{
        ...
        try {
            ...
            const  response = await ApiConnection.delete(`fantasy-game-admin/${rowId}`)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setdeletemodal(false)
    ...
    const  DownLoadExcel = async () =>{
        ...
        try{
            const  response = await ApiConnection.get('fantasy-game-admin/excel')
            if(response?.status == 200){
  ...
  return (
    <div>
    {pageloading && <Loader/>}
<DashboardHeader title="Fantasy Game Admin List" />
<AdminMenu />
  ...
  <div className="dashboard-panel">
        ...
        <div className='form-group w-50 d-flex'>
            ...
            <div className='form-group'>
                <ul>
                    <li>
                        <NavLink to="/admin/fantacy-game-admin/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add fantacy admin</NavLink>
                    </li>
                    <li>
...
<th>League Name</th>
    ...
    <tbody>
       ...
       FilterResult&&FilterResult.map((list, index)=>{
        return (
            <tr key={index}>
             <td>{list?.club[0]?.club_name}</td>
            <td>{list?.first_name + ' ' + list?.last_name}</td>
            <td>{list?.email}</td>
            <td>{list?.phone}</td>
         
            <td>
                <NavLink to={`/admin/fantacy-game-admin/view/${list?.id}`} className="btn btn-sm btn-outline-primary"><i className="fa-solid fa-eye"></i></NavLink>
                <NavLink to={`/admin/fantacy-game-admin/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>
...
export default FantacyGameAdmin
...
const MatchSchedule = () => {
    ...
    const GetData = async (pagenumber, text)=>{
        let endpoint = ''
        if(text !== ''){
            endpoint = `match?search=${text}`
        } else endpoint = `match?page=${pagenumber}`
    ...
    const StatusHandler = async () =>{
        ...
        try {
           ...
           data.append('status', status == 1 ? 0 : 1);
            ...
            const  response = await ApiConnection.post(`match/change-status/${rowId}`, data)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setstatusmodal(false)
    ...
    const DeleteHandler = async () =>{
        ...
        try {
            ...
            const  response = await ApiConnection.delete(`match/${rowId}`)
            if(response?.status == 200){
                GetData()
                setloading(false)
                setdeletemodal(false)
    ...
    const  DownLoadExcel = async () =>{
        ...
        try{
            const  response = await ApiConnection.get('match/excel')
            if(response?.status == 200){
    ...
    const paginateHandle = (number) =>{
      ...
      function scoreboardHandle(lid, teamOne, teamTwo){
        setlistId(lid)
        setteamOneId(teamOne)
        setteamTwoId(teamTwo)
        setscoreboardModal(true)
      }
      ...
      const Submitscore = ()=>{
        if(TeamHandle == ''){
            ...
            navigate("/admin/scorecard", { state: { matchId: listId, teamOneId: teamOneId, teamTwoId:teamTwoId, selectTeam:TeamHandle
  ...
  return (
    <div>
    {pageloading && <Loader/>}
<DashboardHeader title="Match Schedule Listing" />
<AdminMenu />
  ...
  <div className="dashboard-panel">
        ...
        <div className='form-group w-50 d-flex'>
            ...
            <div className='form-group'>
                <ul>
                    <li>
                        <NavLink to="/admin/match-schedule/add" className='btn  btn-success' ><i className="fa-solid fa-plus"></i> Add Match</NavLink>
                    </li>
                    <li>
...
<thead>
    ...
    <tbody>
       ...
       FilterResult&&FilterResult.map((list, index)=>{
        return (
            <tr key={index}>
            <td>{list?.tournament?.tournament_name}</td>
            <td>{moment(list?.match_date).format('MM-DD-YYYY')}</td>
            <td>{ moment(new Date('2024-01-30' + ' ' + list?.match_start_time)).format('LT')  }  </td>
            <td>{list?.team_1?.team_name}</td>
            <td>{list?.team_2?.team_name}</td>

            <td>
                <button className="btn btn-sm btn-outline-primary mr-2" onClick={()=>scoreboardHandle(list?.id,list?.team1_id, list?.team2_id)}>Update Scorecard</button>
                {/* <NavLink to={`/admin/scorecard/${list?.id}/${list?.team1_id}/${list?.team2_id}`} className="btn btn-sm btn-outline-primary mr-2">Update Scorecard</NavLink> */}
                <NavLink to={`/admin/match-schedule/view/${list?.id}`} className="btn btn-sm btn-outline-primary"><i className="fa-solid fa-eye"></i></NavLink>
                <NavLink to={`/admin/match-schedule/edit/${list?.id}`} className="btn btn-sm btn-outline-warning ml-2"><i className="fa-solid fa-pen-to-square"></i></NavLink>
  ...
  >
    ...
    Team Batting First
        ...
        >
            ...
            <option value={teamOneId}>Team A</option>
            <option value={teamTwoId}>Team B</option>
        </select>


        <ul className='d-flex mt-4 justify-content-center'>
            <li>
                <button className='btn btn-success mr-2' onClick={Submitscore}>Submit</button>
            </li>
            <li>
                <button className='btn btn-outline-danger ml-2' onClick={()=>setscoreboardModal(false)}>Cancel</button>
            </li>
        </ul>
    </Modal.Body>
